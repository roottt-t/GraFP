...
# Setup
python3 setup_config.py --noise_dir=$2/noise --ir_dir=$2/ir
eval=$(basename $1)
python3 setup_icassp.py --test_dir=$1 --noise_dir=$2/noise --ir_dir=$2/ir --eval_type=$eval
...
if [[ $eval == "fma_medium" ]]; then
    ...
    do
        ...
        python3 test_fp.py --query_lens=1,2,3,5 --n_query_db=500 \
                            --test_snr=$snr --text=sanir_fma_medium_$snr \
                            --test_ids=data/medeval_ids.npy --model=tc_29
    done
    
elif [[ $eval == "fma_large" ]]; then
    for snr in 20 15 10 5 0
    do
        echo "Test with SNR = $snr"
        python3 test_fp.py --query_lens=1,2,3,5 --n_query_db=500 --test_dir=data/fma_large.json \
